...
var API=(()=>{const b='/api/v1/store';return{
headers:()=>({'Content-Type':'application/json','Authorization':'Bearer '+(STATE.token||'')}),
async login(u,p){const r=await fetch(b+'/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:u,password:p})});return r.json()},
async getProducts(){const r=await fetch(b+'/products',{headers:this.headers()});return r.json()},
async searchProducts(q){const r=await fetch(b+'/products/search?q='+encodeURIComponent(q)+'&limit=50',{headers:this.headers()});return r.json()},
async getByBarcode(code){const r=await fetch('/api/v1/store/pos/products/barcode/'+encodeURIComponent(code),{headers:{'Content-Type':'application/json'}});return r.json()},
async recordSale(d){if(STATE.isOffline)return this._saveOffline(d);const r=await fetch('/api/v1/store/pos/sales',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});const res=await r.json();if(!r.ok&&res.error&&(res.error.includes('timeout')||r.status>=500))return this._saveOffline(d);if(!r.ok&&res.error)throw new Error(res.error);return res},
async _saveOffline(d){const p=JSON.parse(localStorage.getItem('pos_pending')||'[]');p.push({...d,_offlineId:Date.now(),_createdAt:new Date().toISOString()});localStorage.setItem('pos_pending',JSON.stringify(p));return{sale:{receipt_number:'OFFLINE-'+Date.now(),total:d.total||0},_offline:true}},
async getPosProducts(){const r=await fetch('/api/v1/store/pos/products?_='+Date.now(),{headers:{'Content-Type':'application/json'}});return r.json()},
async recordPurchase(d){const r=await fetch('/api/v1/store/pos/purchases',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});return r.json()},
async getPosPurchases(){const r=await fetch('/api/v1/store/pos/purchases',{headers:{'Content-Type':'application/json'}});return r.json()},
async syncPending(){const p=JSON.parse(localStorage.getItem('pos_pending')||'[]');if(!p.length)return;const s=[];for(const x of p){try{const r=await fetch(b+'/sales',{method:'POST',headers:this.headers(),body:JSON.stringify(x)});if(!r.ok)s.push(x)}catch{s.push(x)}}localStorage.setItem('pos_pending',JSON.stringify(s))},
...
function showApp(){document.getElementById('login-screen').classList.add('hidden');document.getElementById('pos-app').classList.add('active');const n=STATE.user?.display_name||STATE.user?.username||'Gerant';document.getElementById('sidebar-avatar').textContent=n.charAt(0).toUpperCase();const storeName=STATE.user?.store_name||'ROYAL CHAUSSURES';const sn=document.getElementById('store-name');if(sn){sn.textContent=storeName}const title=document.querySelector('.brand span');if(title){title.textContent=storeName}const pt=document.getElementById('page-title');if(pt){pt.textContent=storeName+' - POS'}const lt=document.getElementById('login-title');if(lt){lt.textContent=storeName}switchView('sale')}

function logout(){STATE.token=null;STATE.user=null;STATE.cart=[];localStorage.removeItem('pos_token');localStorage.removeItem('pos_user');document.getElementById('pos-app').classList.remove('active');document.getElementById('login-screen').classList.remove('hidden');document.getElementById('login-user').value='';document.getElementById('login-pass').value='';document.getElementById('login-btn').disabled=false;document.getElementById('login-btn').innerHTML='<i class="fas fa-sign-in-alt"></i> Connexion';const sn=document.getElementById('store-name');if(sn)sn.textContent='ROYAL CHAUSSURES';const title=document.querySelector('.brand span');if(title)title.textContent='ROYAL CHAUSSURES'}
...
(function(){const originalComplete=completeSale;async function doSale(){try{const total=STATE.cart.reduce((s,i)=>s+(i.price*i.qty),0);const disc=parseFloat(document.getElementById('sale-remise')?.value)||0;const final=Math.max(0,total-disc);const items=STATE.cart.map(i=>({product_id:i.id,product_name:i.name,quantity:i.qty,unit_price:i.price,total_price:i.price*i.qty}));const payload={store_id:1,cashier:STATE.user?.username||'caisse',subtotal:total,discount:disc,tax:0,total:final,payment_method:STATE.selectedPayment,notes:'',items:items};const res=await API.recordSale(payload);const receipt=res?.sale?.id?'POS-'+res.sale.id:'POS-'+Date.now();STATE.lastSale={receipt,total:final,items:[...STATE.cart],payment:STATE.selectedPayment};document.getElementById('payment-form').style.display='none';document.getElementById('payment-success').style.display='block';document.getElementById('success-total').textContent=final.toLocaleString()+' DA';document.getElementById('success-receipt').textContent='Facture no: '+receipt;STATE.cart=[];updateSaleUI()}catch(e){showToast('Erreur: '+e.message,'error')}}document.getElementById('btn-complete-sale').onclick=doSale})();
...
(function(){const orig=switchView;switchView=function(v){orig(v);if(v==='products')renderProductsTable();if(v==='purchases-list')loadPurchaseList()}})();
